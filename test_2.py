...
def print_solution(data, manager, routing, solution):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        data['vehicle_names'][vehicle_id]
        plan_output = 'Route for vehicle {}:\n'.format(data['vehicle_names'][vehicle_id])
        route_distance = 0
        route_load_l = 0
        route_load_m = 0
        ...
        plan_output += ' {0} Entrega({1} kg, {2}m)\n'.format(data['cities_names'][manager.IndexToNode(index)],
                                                 route_load_m, route_load_l)
